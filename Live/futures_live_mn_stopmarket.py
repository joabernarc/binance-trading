...
def keltner_channels(last_close):
    ...
    try:
        ...
        if not isLong and not isShort:
            time.sleep(3)
        klines = client.get_historical_klines("agixusdt", Client.KLINE_INTERVAL_5MINUTE, "1 March, 2023", klines_type=HistoricalKlinesType.FUTURES)
...
def user_callback(userDataUpdate):
    ...
    if userDataUpdate['e'] == 'ACCOUNT_UPDATE':
        ...
        quote_order_quantity = (float(balance) * 20) * 0.95
    ...
    if userDataUpdate['e'] == 'ORDER_TRADE_UPDATE': 
        ...
        elif userDataUpdate['o']['S'] == SIDE_SELL and userDataUpdate['o']['R'] == True and userDataUpdate['o']['ot'] == FUTURE_ORDER_TYPE_MARKET: #and userDataUpdate['o']['ot'] == FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET:
                open_new_order_asap = False
                isLong = False
                cancel_all_orders = client.futures_cancel_all_open_orders(symbol=TRADE_SYMBOL)
                print('canceled orders:', cancel_all_orders)
        elif userDataUpdate['o']['S'] == SIDE_BUY and userDataUpdate['o']['R'] == True and userDataUpdate['o']['ot'] == FUTURE_ORDER_TYPE_MARKET: #and userDataUpdate['o']['ot'] == FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET:
                open_new_order_asap = False
                isShort = False
                cancel_all_orders = client.futures_cancel_all_open_orders(symbol=TRADE_SYMBOL)
                print('canceled orders:', cancel_all_orders)
        ...
        if last_order:
            if isLong:
                ...
                if retry_count > max_retry_count:
                    if isPartial:
                        market_order_succeeded = order(SIDE_SELL, cum_partial_quantity, TRADE_SYMBOL, round((highest - 0.0001), 4), FUTURE_ORDER_TYPE_MARKET, True)
                    else:
                        market_order_succeeded = order(SIDE_SELL, last_trade_quantity, TRADE_SYMBOL, round((highest - 0.0001), 4), FUTURE_ORDER_TYPE_MARKET, True)
                elif isPartial:
                    if retry_count == 1:
                        retry_count = retry_count+1
                    order_succeeded = order(SIDE_SELL, cum_partial_quantity, TRADE_SYMBOL, round((highest - 0.0001), 4), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
                else:
                    order_succeeded = order(SIDE_SELL, last_trade_quantity, TRADE_SYMBOL, round((highest - 0.0001), 4), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
            elif isShort:
                retry_count = retry_count + 1
                print('retry count:', retry_count)
                if retry_count > max_retry_count:
                    if isPartial:
                        market_order_succeeded = order(SIDE_BUY, cum_partial_quantity, TRADE_SYMBOL, round((lowest + 0.0001), 4), FUTURE_ORDER_TYPE_MARKET, True)
                    else:
                        market_order_succeeded = order(SIDE_BUY, last_trade_quantity, TRADE_SYMBOL, round((lowest + 0.0001), 4), FUTURE_ORDER_TYPE_MARKET, True)
                elif isPartial:
                    if retry_count == 1:
                        retry_count = retry_count+1
                    order_succeeded = order(SIDE_BUY, cum_partial_quantity, TRADE_SYMBOL, round((lowest + 0.0001), 4), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
                else:
                    order_succeeded = order(SIDE_BUY, last_trade_quantity, TRADE_SYMBOL, round((lowest + 0.0001), 4), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
            else:
    ...
    if open_new_order_asap:
        if isLong:
            if not isPartial:
                ...
                print('retry count:', retry_count)
            order_succeeded = order(SIDE_SELL, last_trade_quantity, TRADE_SYMBOL, round((highest - 0.0001), 4), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
            stop_price_long = round((lowest * 0.993), 4)
            ...
            if not isPartial:
                ...
                print('retry count:', retry_count)
            order_succeeded = order(SIDE_BUY, last_trade_quantity, TRADE_SYMBOL, round((lowest + 0.0001), 4), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
            stop_price_short = round((highest * 1.007), 4)
