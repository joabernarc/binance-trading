...
def keltner_channels(last_close):
    ...
    try:
        ...
        klines = client.get_historical_klines("agixusdt", Client.KLINE_INTERVAL_5MINUTE, "10 March, 2023", klines_type=HistoricalKlinesType.FUTURES)
...
def user_callback(userDataUpdate):
    ...
    if userDataUpdate['e'] == 'ACCOUNT_UPDATE':
        ...
        quote_order_quantity = (float(balance) * 10) * 0.1
...
def kline_callback(kline):
    global open_new_order_asap, isPartial, quantity, last_order, retry_count, close, lowest, highest, lows, highs, isLong, isShort, max_retry_count, last_trade_quantity, quote_order_quantity, above_upper_bollinger, below_lower_bollinger, cum_partial_quantity, take_profit_order
    ...
    if is_candle_closed:
        ...
        if isLong:
            ...
            if retry_count > max_retry_count:
                if isPartial:
                    market_order_succeeded = order(SIDE_SELL, cum_partial_quantity, TRADE_SYMBOL, max(highs), FUTURE_ORDER_TYPE_MARKET, True)
                else:
                    market_order_succeeded = order(SIDE_SELL, last_trade_quantity, TRADE_SYMBOL, max(highs), FUTURE_ORDER_TYPE_MARKET, True)
            elif isPartial:
                if retry_count == 1:
                    retry_count = retry_count+1
                order_succeeded = order(SIDE_SELL, cum_partial_quantity, TRADE_SYMBOL, max(highs), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
            else:
                order_succeeded = order(SIDE_SELL, last_trade_quantity, TRADE_SYMBOL, max(highs), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
        elif isShort:
            retry_count = retry_count + 1
            print('retry count:', retry_count)
            if retry_count > max_retry_count:
                if isPartial:
                    market_order_succeeded = order(SIDE_BUY, cum_partial_quantity, TRADE_SYMBOL, min(lows), FUTURE_ORDER_TYPE_MARKET, True)
                else:
                    market_order_succeeded = order(SIDE_BUY, last_trade_quantity, TRADE_SYMBOL, min(lows), FUTURE_ORDER_TYPE_MARKET, True)
            elif isPartial:
                if retry_count == 1:
                    retry_count = retry_count+1
                order_succeeded = order(SIDE_BUY, cum_partial_quantity, TRADE_SYMBOL, min(lows), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
            else:
                order_succeeded = order(SIDE_BUY, last_trade_quantity, TRADE_SYMBOL, min(lows), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
        else:
            if above_upper_keltner: #and not above_upper_bollinger:
                order_succeeded = order(SIDE_BUY, quantity, TRADE_SYMBOL, min(lows), FUTURE_ORDER_TYPE_LIMIT, False)
                retry_count = 0
            if below_lower_keltner: #and not below_lower_bollinger:
                order_succeeded = order(SIDE_SELL, quantity, TRADE_SYMBOL, max(highs), FUTURE_ORDER_TYPE_LIMIT, False)
                retry_count = 0
    ...
    if open_new_order_asap:
        if isLong:
            if not isPartial:
                ...
                print('retry count:', retry_count)
            order_succeeded = order(SIDE_SELL, last_trade_quantity, TRADE_SYMBOL, max(highs), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
            stop_price_long = round((min(lows) * 0.995), 1)
            ...
            if not isPartial:
                ...
                print('retry count:', retry_count)
            order_succeeded = order(SIDE_BUY, last_trade_quantity, TRADE_SYMBOL, min(lows), FUTURE_ORDER_TYPE_TAKE_PROFIT_MARKET, True)
            stop_price_short = round((max(highs) * 1.005), 1)
